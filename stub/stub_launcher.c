 ...
 *
 * Usage:
 *   stub_launcher <account_name> <characters_count> <ticket> <game_lang>
 * <game_path>
 *
...
int main(const int argc, char *argv[]) {
  if (argc != 7) {
    fprintf(stderr, "Usage: stub_launcher <account_name> <characters_count> "
                    "<ticket> <game_lang> <game_path> <server_list_url>\n");
    return -1;
  }
  ...
  const int exit_code = run_game(account_name, characters_count, ticket,
                                 game_lang, game_path, server_list_in);

...
BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam)
{
    ...
    if (GetClassNameA(hwnd, className, sizeof(className))) {
        ...
        if (GetWindowTextA(hwnd, windowTitle, sizeof(windowTitle))) {
            ...
            printf("Window Handle: 0x%08p, Class Name: %s, Window Title: [No Title]\n",
                   (void*)hwnd, className);
        }
    }
...
{
    snprintf(g_credentials.account_name,     sizeof(g_credentials.account_name),     "%s", account_name);
    snprintf(g_credentials.characters_count, sizeof(g_credentials.characters_count), "%s", characters_count);
    snprintf(g_credentials.ticket,           sizeof(g_credentials.ticket),           "%s", ticket);
    snprintf(g_credentials.game_lang,        sizeof(g_credentials.game_lang),        "%s", game_lang);
    snprintf(g_credentials.game_path,        sizeof(g_credentials.game_path),        "%s", game_path);
}
...
{
    return g_credentials.account_name;
}
...
{
    return g_credentials.game_lang;
}
...
{
    return g_credentials.game_path;
}
...
/* Event handles */
static HANDLE g_gameStatusEvent = NULL;    /**< Signaled when game status changes. */
static HANDLE g_windowCreatedEvent = NULL; /**< Signaled when window is created. */

/**
 * @brief A variable that is set atomically to synchronize loading of the launcher "window" and the game client.
 */
...
static WCHAR *to_wstring(const char *s)
{
    ...
    info("Input string: %s, input length: %lu, out_sz: %d",
         s, (unsigned long)strlen(s), len);
 ...
 *
 * @param recipient HWND cast in the wParam, i.e., (HWND)wParam from your wnd_proc.
 ...
 */
static void send_response_message(WPARAM recipient,
                                  HWND sender,
                                  DWORD event_id,
                                  const void *data,
                                  size_t length)
{
    ...
    LRESULT result = SendMessageW((HWND)recipient,
                                  WM_COPYDATA,
                                  (WPARAM)sender,
                                  (LPARAM)&cds);
...
/**
 * @brief Handles a request for the account name. Responds with a UTF-16 string (no null terminator).
 *
...
static void handle_account_name_request(WPARAM recipient, HWND sender)
{
    ...
    /* I would prefer to measure this against wbuf, but since wcslen() seems to return an incorrect length in
       certain environments with winelib, strlen() against the input is safer here. */
    size_t cbSize = strlen(account_name) * sizeof(WCHAR);

    /* Send the wide chars as raw bytes WITHOUT null terminator and with event_id=2 */
...
/**
 * @brief Handles a request for the session ticket. Sends it back as UTF-8 bytes (no null terminator).
 *
 * @param recipient The HWND of the recipient (wParam).
 * @param sender    The local HWND (hWnd).
 */
...
/**
 * @brief Handles a request for the server list. Calls get_server_list() and sends
 *        the resulting data (if any) back via WM_COPYDATA.
 *
 * @param recipient The HWND of the recipient (wParam).
 * @param sender    The local HWND (hWnd).
 */
...
{
    size_t out_size = 0;
    uint8_t *data = get_server_list(&out_size, server_list_url_global, get_characters_count());
 ...
 */
static void handle_game_start(WPARAM recipient,
                              HWND sender,
 ...
 * @param payload     Pointer to the payload containing world information (if any).
 * @param payload_len Length of the payload in bytes.
 */
static void handle_enter_lobby_or_world(WPARAM recipient,
                                        HWND sender,
                                        const unsigned char *payload,
                                        size_t payload_len)
{
    if (!payload || payload_len == 0) {
        ...
        /* Ensure that payload_len is reasonable to prevent excessive memory allocation */
        if (payload_len > 1024) {
            error_log("Payload size too large in handle_enter_lobby_or_world: %zu", payload_len);
            send_response_message(recipient, sender, 8, NULL, 0);
            return;
        }
...
/**
 * @brief Window procedure to handle messages related to game events and data transfer.
 *
...
/**
 * @brief Creates an invisible launcher window and enters a message loop until WM_QUIT.
 */
...
----------------------------------------------------*/
/**
 * @brief Launches a process (the game executable) with the specified path and arguments.
 *
 * @param path Full path to the executable.
 * @param args Command-line arguments to pass.
 * @return PROCESS_INFORMATION struct describing the newly created process/thread.
 */
static PROCESS_INFORMATION launch_process(const char *path, const char *args)
{
    ...
    size_t neededLen = path_len + args_len + 4; /* Extra space for quotes, space, null terminator */
    ...
    if (!success) {
        ...
        error_log("Failed to create process. GetLastError=%lu", (unsigned long)error_code);

        char error_msg[512];
        DWORD flags = FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS;
        DWORD langId = MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT);
        DWORD msgLen = FormatMessageA(flags,
                                      NULL,
                                      error_code,
                                      langId,
                                      error_msg,
        ...
        info("Process created successfully: PID=%lu", (unsigned long)pi.dwProcessId);
    }
...
/**
 * @brief Waits indefinitely for the given process to exit, returning its exit code.
 *
...
/**
 * @brief Launches the game, creates the TERA launcher window, then waits for the
 *        game process to exit. Cleans up and signals WM_GAME_EXITED on completion.
 *
             ...
             const char *server_list_url)
{
    ...
    info("Set credentials: Account=%s, CharCount=%s, Ticket=%s, Lang=%s, GamePath=%s, ServerListURL=%s",
         get_account_name(),
    ...
    if (wait_res != WAIT_OBJECT_0) {
        error_log("WaitForSingleObject() on g_windowCreatedEvent failed, %lu", (unsigned long)GetLastError());
    }

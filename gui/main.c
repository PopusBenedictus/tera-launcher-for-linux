...
/**
 * @brief Used to capture auth info when a user logs into the TERA server. TODO: implement bounds checks here because
 *        we could easily exceed these limits and cause a crash.
 */
typedef struct {
    char user_no[256];
    char auth_key[256];
...
/**
 * @brief Used to help with the manual window dragging callback and storing the state thereof.
 */
typedef struct {
    gboolean dragging;
} DragData;
...
/**
 * @brief Used to store all the GUI widgets and possibly some state relating to window moves and other things.
 */
typedef struct {
    GtkWidget* window;
...
/**
 * @brief Holds a copy of the service name that is displayed in the patch window footer.
 */
static char service_name_global[1024] = {0};

/**
 * @brief Used to store the final update thread message, if any, to update progress bar label
 *        when the update resources are being thrown out.
 */
static char update_finish_message[1024] = {0};

/**
 * @brief Holds a reference to the GUI stylesheet from the embedded resources. Cannot be const due to loss of
 *        qualifiers on assignment, but should not be free'd.
 */
 ...
 */
static bool do_login(const char* username, const char* password, LoginData* out)
{
    ...
    // Prepare POST data
    char postfields[512];
    snprintf(postfields, sizeof(postfields), "login=%s&password=%s", username, password);
                ...
                if (json_is_boolean(jReturn) && json_is_string(jMsg)) {
                    ...
                    if (retVal && strcmp(msg, "success") == 0) {
                        ...
                        if (json_is_number(jUserNo) && json_is_string(jAuthKey)) {
                            ...
                            const char* charCnt = json_is_string(jCharCnt) ? json_string_value(jCharCnt) : "0";

                            snprintf(out->user_no, sizeof(out->user_no), "%.0f", userNoVal);
                            strncpy(out->auth_key, authKey, sizeof(out->auth_key) - 1);
                            strncpy(out->character_count, charCnt, sizeof(out->character_count) - 1);
                        ...
                        const char* reason = json_string_value(json_object_get(root, "Msg"));
 ...
 * @return Pointer to the GdkTexture, or NULL on failure.
 */
static GdkTexture* load_subimage(const char* resource_path,
                                 int sub_x, int sub_y,
                                 int sub_w, int sub_h)
{
    ...
    GdkPixbuf* sub_pixbuf = gdk_pixbuf_new_subpixbuf(full_pixbuf, sub_x, sub_y, sub_w, sub_h);
...
static GtkWidget* create_login_overlay(LauncherData* ld)
{
    ...
    gtk_style_context_add_provider_for_display(gtk_widget_get_display(overlay),
    ...
    // Form (Username Field)
    GdkTexture* form_uname_tex = load_subimage("/com/tera/launcher/form.png", 0, 0, 220, 50);
    if (form_uname_tex) {
        GtkWidget* form_uname_pic = gtk_picture_new_for_paintable(GDK_PAINTABLE(form_uname_tex));
    ...
    // Form (Password Field)
    GdkTexture* form_passwd_tex = load_subimage("/com/tera/launcher/form.png", 0, 50, 220, 50);
    if (form_passwd_tex) {
        GtkWidget* form_passwd_pic = gtk_picture_new_for_paintable(GDK_PAINTABLE(form_passwd_tex));
    ...
    // Login Button
    GdkTexture* login_sub = load_subimage("/com/tera/launcher/btn-auth.png", 0, 0, 224, 69);
    ...
    // Close Button
    GdkTexture* close_tex = load_subimage("/com/tera/launcher/btn-close1.png", 0, 0, 22, 22);
...
/**
 * @brief Creates the patch overlay containing play, logout, and repair buttons, along with progress bars.
 *
 * @param ld Pointer to the LauncherData structure.
 * @return Pointer to the created GtkWidget overlay.
 */
static GtkWidget* create_patch_overlay(LauncherData* ld)
{
    ...
    const size_t footer_sz = snprintf(footer_str, sizeof(footer_str), "© %i %s. All Rights Reserved.", year, service_name_global);
    if (footer_sz <= 0)
    ...
    // Logout Button
    GdkTexture* logout_tex = load_subimage("/com/tera/launcher/btn-logout.png", 0, 0, 18, 18);
    ...
    // Play Button
    GdkTexture* play_tex = load_subimage("/com/tera/launcher/btn-game-start.png", 0, 0, 240, 90);
    ...
    gtk_progress_bar_set_show_text(GTK_PROGRESS_BAR(ld->update_repair_progress_bar), TRUE);
    gtk_progress_bar_set_text(GTK_PROGRESS_BAR(ld->update_repair_progress_bar), "Ready");
    gtk_widget_add_css_class(ld->update_repair_progress_bar, "repair-progress-bar");
    gtk_overlay_add_overlay(GTK_OVERLAY(overlay), ld->update_repair_progress_bar);
    gtk_progress_bar_set_ellipsize(GTK_PROGRESS_BAR(ld->update_repair_progress_bar), PANGO_ELLIPSIZE_END);
    ...
    gtk_progress_bar_set_show_text(GTK_PROGRESS_BAR(ld->update_repair_download_bar), TRUE);
    gtk_progress_bar_set_text(GTK_PROGRESS_BAR(ld->update_repair_download_bar), "");
    gtk_widget_add_css_class(ld->update_repair_download_bar, "repair-progress-bar");
    gtk_overlay_add_overlay(GTK_OVERLAY(overlay), ld->update_repair_download_bar);
    gtk_progress_bar_set_ellipsize(GTK_PROGRESS_BAR(ld->update_repair_download_bar), PANGO_ELLIPSIZE_END);
    gtk_widget_set_size_request(ld->update_repair_download_bar, 450, 40);
    gtk_widget_set_hexpand(ld->update_repair_download_bar, FALSE);

    // Repair Button
    GdkTexture* rep_tex = load_subimage("/com/tera/launcher/repair-btn.png", 0, 0, 50, 50);
    ...
    // Close Button
    GdkTexture* close_patch_tex = load_subimage("/com/tera/launcher/btn-close.png", 0, 0, 22, 22);
    ld->close_patch_btn = gtk_button_new();
    if (close_patch_tex) {
        GtkWidget* cpic = gtk_picture_new_for_paintable(GDK_PAINTABLE(close_patch_tex));
        gtk_picture_set_content_fit(GTK_PICTURE(cpic), GTK_CONTENT_FIT_FILL);
 ...
 * @param response_id The response chosen by the user (close, delete-event, etc.).
 * @param user_data   Pointer to the GtkApplication to be quit.
 */
static void
on_fatal_error_dialog_response(GtkDialog *dialog, gint response_id, gpointer user_data)
{
    /* Destroy the dialog */
...
/**
 * @brief Show a fatal error dialog (for GTK4) with the given message, then quit the app.
 *
 ...
 */
static void
show_fatal_error_dialog_and_exit(GtkApplication *app, const char *message)
{
    /*
    ...
    g_signal_connect(dialog,
                     "response",
                     G_CALLBACK(on_fatal_error_dialog_response),
                     app);
 ...
 *
 * Looks up the specified resource, reads it as JSON, and returns the jansson JSON root object.
 * On any error, it displays a fatal error dialog (using the provided GtkApplication) and
 * returns NULL.
 *
 ...
 */
static json_t*
load_launcher_config_json(GtkApplication *app, const char *resource_path)
{
    GError *error = nullptr;
    GBytes *launcher_config_gbytes = g_resources_lookup_data(resource_path, 0, &error);

    if (!launcher_config_gbytes) {
        show_fatal_error_dialog_and_exit(app,
            g_strdup_printf("Failed to load config file: %s",
    ...
    if (!data) {
        show_fatal_error_dialog_and_exit(app,
            g_strdup_printf("Failed to load embedded configuration data: %s",
                            error ? error->message : "Unknown error"));
        if (error) {
            g_error_free(error);
        }
    ...
    if (!config_json) {
        show_fatal_error_dialog_and_exit(app, "Could not parse launcher configuration JSON.");
        if (error) {
            g_error_free(error);
        }
        return nullptr;
...
/**
 * @brief Retrieve a string value by key from the JSON object and copy it into the given buffer.
 *
 * If the key is missing or not a string, a fatal error dialog is shown and the application quits.
 *
 * @param app                  The GtkApplication to use for showing fatal errors.
                                  ...
                                  char *destination)
{
    ...
    if (!field) {
        show_fatal_error_dialog_and_exit(app,
            g_strdup_printf("Could not parse key: %s", key));
    }

    if (!json_is_string(field)) {
        show_fatal_error_dialog_and_exit(app,
            g_strdup_printf("Key '%s' is not a valid string.", key));
    }
 ...
 *
 * Reads JSON from /com/tera/launcher/launcher-config.json, populates the global config
 * variables, and checks them for correctness. On failure, it shows an error dialog
 * (via @p app) and quits the application.
 *
 * @param app The GtkApplication instance used for showing fatal error dialogs.
 * @return gboolean TRUE on success (the function returns normally). In the event
 *                  of an error, a dialog is shown and the app quits, so effectively
 *                  this function only returns FALSE if something interrupts
 *                  the flow before the dialog.
 */
static gboolean launcher_init_config(GtkApplication *app)
{
    json_t *launcher_config_json = load_launcher_config_json(app, "/com/tera/launcher/launcher-config.json");
    if (!launcher_config_json) {
        return FALSE;
    }

    parse_and_copy_string(app, launcher_config_json, "public_patch_url",  patch_url_global);
    parse_and_copy_string(app, launcher_config_json, "auth_url",          auth_url_global);
    parse_and_copy_string(app, launcher_config_json, "server_list_url",   server_list_url_global);
    parse_and_copy_string(app, launcher_config_json, "wine_prefix_name",  wineprefix_global);
    ...
    parse_and_copy_string(app, launcher_config_json, "game_lang",     game_lang_global);
    parse_and_copy_string(app, launcher_config_json, "service_name",  service_name_global);
...
static gboolean progress_bar_final_callback(gpointer data) {
    ...
    if (strlen(update_finish_message) != 0) {
        ...
        gtk_progress_bar_set_text(td->update_data.progress_bar, update_finish_message);
 ...
 */
static void update_progress_callback(double progress, const char* message, gpointer user_data)
{
    ...
    g_idle_add_full(G_PRIORITY_HIGH_IDLE, progress_bar_callback, ut_data_ref(td), (GDestroyNotify)ut_data_unref);
}
 ...
 * @param user_data Pointer to LauncherData.
 */
static void update_download_progress_callback(double progress, const char* message, gpointer user_data)
{
    UpdateThreadData* td = user_data;
    ...
    // Schedule the update in the main thread
    g_idle_add_full(G_PRIORITY_HIGH_IDLE, download_progress_bar_callback, ut_data_ref(td), (GDestroyNotify)ut_data_unref);
}
...
static gpointer update_thread_func(gpointer data)
{
    ...
    // Step 2: Get files to update (may become repair task if conditions align to require it or is requested)
    GList* files_to_update;
    if (ut_data->repair_requested)
        files_to_update = get_files_to_repair(update_data, update_progress_callback, ut_data);
    else
        files_to_update = get_files_to_update(update_data, update_progress_callback, ut_data);

    if (!files_to_update) {
        ut_data->play_button_enabled = TRUE;
        ut_data->repair_button_enabled = TRUE;
        strcpy(update_finish_message, "Game is up to date.");
        g_idle_add_full(G_PRIORITY_HIGH_IDLE, progress_bar_final_callback, ut_data_ref(ut_data), (GDestroyNotify)ut_data_unref);
        g_idle_add_full(G_PRIORITY_HIGH_IDLE, button_status_callback, ut_data_ref(ut_data), (GDestroyNotify)ut_data_unref);
        return nullptr;
    }

    // Step 3: Download all files
    if (!download_all_files(update_data, files_to_update, update_progress_callback, update_download_progress_callback, ut_data)) {
        ut_data->play_button_enabled = TRUE;
        ut_data->repair_button_enabled = TRUE;
        g_idle_add_full(G_PRIORITY_HIGH_IDLE, button_status_callback, ut_data_ref(ut_data), (GDestroyNotify)ut_data_unref);
...
/**
 * @brief Initiates the update process by checking for updates and downloading them.
 *
 * @param ld Pointer to the LauncherData structure.
 * @param do_repair If set to TRUE, a full repair operation is carried out rather than an update.
 */
static void start_update_process(LauncherData* ld, bool do_repair)
{
    ...
    gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(ld->update_repair_progress_bar), 0.0);
    if (do_repair)
        gtk_progress_bar_set_text(GTK_PROGRESS_BAR(ld->update_repair_progress_bar), "Starting repair...");
    else
        gtk_progress_bar_set_text(GTK_PROGRESS_BAR(ld->update_repair_progress_bar), "Checking for updates...");
    ...
    if (!thread_data) {
        ...
        if (do_repair)
            gtk_progress_bar_set_text(GTK_PROGRESS_BAR(ld->update_repair_progress_bar), "Repair failed.");
        else
            gtk_progress_bar_set_text(GTK_PROGRESS_BAR(ld->update_repair_progress_bar), "Update failed.");
    ...
    // Share patch URL and game path in UpdateData for update functions to do their thing.
    ...
    thread_data->ld = ld;
    thread_data->update_data.progress_bar = GTK_PROGRESS_BAR(ld->update_repair_progress_bar);
    thread_data->update_data.download_progress_bar = GTK_PROGRESS_BAR(ld->update_repair_download_bar);
    thread_data->repair_requested = do_repair;

    // Create and start the thread
    GThread* thread = g_thread_new("update_thread", update_thread_func, thread_data);
    if (!thread) {
...
void game_exit_callback(const int exit_code, const void* user_data) {
    ...
    g_idle_add_full(G_PRIORITY_HIGH_IDLE, restore_launcher_callback, cb_data, nullptr);
}
...
static gpointer game_launcher_thread(gpointer data) {
    ...
    // Assemble the launcher path: "Z:" + current working directory with backslashes + "Binaries\\TERA.exe"
    char cwd[PATH_MAX];
    ...
    // Assemble the path. The stub launcher is a winelib app and wants a Windows-like file path.
    char launcher_path[PATH_MAX];
    snprintf(launcher_path, sizeof(launcher_path), "Z:%s\\Binaries\\TERA.exe", cwd);

    // Prepare command-line arguments for stub_launcher
    const char* account_name = launch_data->ld->login_data.user_no;
    const char* characters_count = (launch_data->ld->login_data.character_count[0] ? launch_data->ld->login_data.character_count : "0");
    ...
    gchar* stub_launcher_path = g_build_filename(current_dir, "stub_launcher.exe", nullptr);
    ...
    if (custom_wine_dir && *custom_wine_dir != '\0') {
        ...
        const gchar *old_ld_library_path = g_environ_getenv(envp, "LD_LIBRARY_PATH");
        GString *new_ld_library_path = g_string_new("");

        g_string_append_printf(new_ld_library_path, "%s/lib:%s/lib64", custom_wine_dir, custom_wine_dir);
        ...
        envp = g_environ_setenv(envp, "LD_LIBRARY_PATH", new_ld_library_path->str, TRUE);
        g_string_free(new_ld_library_path, TRUE);

        gchar *wine_loader_path = g_build_filename(custom_wine_dir, "bin", "wine", nullptr);
        envp = g_environ_setenv(envp, "WINELOADER", wine_loader_path, TRUE);
        g_free(wine_loader_path);

        gchar *wine_server_path = g_build_filename(custom_wine_dir, "bin", "wineserver", nullptr);
        envp = g_environ_setenv(envp, "WINESERVER", wine_server_path, TRUE);
        g_free(wine_server_path);

        const gchar *home_dir = g_get_home_dir();
        if (home_dir) {
            gchar *wine_prefix_path = g_build_filename(home_dir, wineprefix_global, nullptr);
        ...
        if (!wine_path) {
            g_warning("System Wine not found on PATH, and TERA_CUSTOM_WINE_DIR not set.");
            g_strfreev(envp);
            return FALSE;
        }
    }
    ...
    g_message("Restore Game Launcher controls after game exit. Exit status: %i", exit_status);
...
static void on_play_clicked(GtkButton* btn, gpointer user_data)
{
    ...
    // Start the thread
    GThread* thread = g_thread_new("game_launcher_thread", game_launcher_thread, (gpointer)launch_data);
    if (!thread) {
...
static void on_login_clicked(GtkButton* btn, gpointer user_data)
{
    ...
    if (do_login(username, password, &temp)) {
        // Store login data
        strncpy(ld->login_data.user_no, temp.user_no, sizeof(ld->login_data.user_no) - 1);
        strncpy(ld->login_data.auth_key, temp.auth_key, sizeof(ld->login_data.auth_key) - 1);
        strncpy(ld->login_data.character_count, temp.character_count, sizeof(ld->login_data.character_count) - 1);

        // Prepare user welcome label on patch screen.
        snprintf(ld->login_data.welcome_label_msg, sizeof(ld->login_data.welcome_label_msg),
                 "Welcome, <b>%s!</b>", username);

        gtk_label_set_markup(GTK_LABEL(ld->welcome_label), ld->login_data.welcome_label_msg);

        g_message("Login success => switch to patch");
        switch_to_patch(ld);
    } else {
        // Display error dialog
        GtkWidget* dialog = gtk_message_dialog_new(GTK_WINDOW(ld->window),
 ...
 * @param user_data Pointer to LauncherData.
 */
static void on_repair_dialog_popped(GtkDialog* dialog, gint response_id, gpointer user_data) {
...
/**
 * @brief Callback function for handling mouse motion events for window dragging.
 *
...
static gboolean on_motion(GtkEventControllerMotion *controller, double x, double y, gpointer user_data) {
    ...
    if ((state & GDK_BUTTON1_MASK) && !data->dragging) {
        ...
        guint32 timestamp = gdk_event_get_time(gtk_event_controller_get_current_event(GTK_EVENT_CONTROLLER(controller)));
        if (!timestamp) {
        ...
        if (!gtk_widget_compute_point(widget, GTK_WIDGET(gtk_widget_get_root(widget)), &source_point, &result_point)) {
            // Unable to compute point
            data->dragging = FALSE;
            gtk_event_controller_reset(GTK_EVENT_CONTROLLER(controller));
            return FALSE;
        }
        gdk_toplevel_begin_move(GDK_TOPLEVEL(surface), gdk_event_get_device(event), button, x_root, y_root, timestamp);
        gtk_event_controller_reset(GTK_EVENT_CONTROLLER(controller));
    } else {
        data->dragging = FALSE;
    }
...
/**
 * @brief Switches the UI from the login overlay to the patch overlay and initiates the update process.
 *
 * @param ld Pointer to the LauncherData structure.
 */
static void switch_to_patch(LauncherData* ld)
...
static void activate(GtkApplication* app, gpointer user_data)
{
    ...
    GError* error = nullptr;
    style_data_gbytes = g_resources_lookup_data("/com/tera/launcher/styles.css", 0, &error);
    if (error) {
    ...
    g_signal_connect(ld->close_login_btn, "clicked", G_CALLBACK(on_close_login_clicked), ld);
    ...
    g_signal_connect(ld->logout_btn,      "clicked", G_CALLBACK(on_logout_clicked), ld);
    g_signal_connect(ld->repair_btn,      "clicked", G_CALLBACK(on_repair_clicked), ld);
    g_signal_connect(ld->close_patch_btn, "clicked", G_CALLBACK(on_close_patch_clicked), ld);

    // Track motion on our window overlays
    gtk_widget_add_controller(ld->login_overlay, GTK_EVENT_CONTROLLER(ld->login_controller));
    gtk_widget_add_controller(ld->patch_overlay, GTK_EVENT_CONTROLLER(ld->patch_controller));
...
int main(const int argc, char** argv)
{
    ...
    GtkApplication* app = gtk_application_new("com.tera.launcher", G_APPLICATION_DEFAULT_FLAGS);

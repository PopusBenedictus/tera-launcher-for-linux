...
/* SQL query to get a count of records for the query above (used to report progress during repair operations) */
...
/* SQL query to get a count of records for the query above (used to report progress during directory
 * tree build operations) */
 ...
 */
static void update_progress(ProgressCallback callback, double progress, const char *message, gpointer user_data)
{
    if (callback)
...
static void print_size(const double bytes, char *out, const size_t sz) {
    ...
    if (bytes < 1024.0 * 1024.0) {
        // Print in KB
        snprintf(out, sz, "%.2f KB", bytes / 1024.0);
    } else {
        // Print in MB
        snprintf(out, sz, "%.2f MB", bytes / (1024.0 * 1024.0));
    }
}

// Helper to print speed in kilobits or megabits
static void print_speed(curl_off_t bytes_per_second, char *out, const size_t sz) {
    ...
    if (bits_per_second < 1024.0 * 1024.0) {
        // Print in kb/s
        snprintf(out, sz, "%.2f kb/s", bits_per_second / 1024.0);
    } else {
        // Print in Mb/s
        snprintf(out, sz, "%.2f Mb/s", bits_per_second / (1024.0 * 1024.0));
    }
}
 ...
 */
static int xfer_progress(void *p, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow) {
    ProgressData *data = p;
    curl_off_t speed;
    ...
    if (now - data->last_update_time >= 0.5) {
        ...
        print_size((double)dltotal, data->download_total, sizeof(data->download_total));
        print_speed(speed, data->download_speed, sizeof(data->download_speed));
        snprintf(data->pbar_label, sizeof(data->pbar_label), "Progress: ( %s / %s ) %s",
            data->download_now, data->download_total, data->download_speed);
        update_progress(data->download_callback, (double)dlnow / (double)dltotal, data->pbar_label, data->user_data);
    }
    return 0;
}
 ...
 * Returns a newly allocated hexadecimal string (which should be freed by the caller),
 * or NULL on error.
 */
 ...
 * get_file_size:
 *
 * Returns the size (in bytes) of the file at 'filepath', or 0 if it cannot be read.
 */
 ...
 * If expected_size is greater than 0, verifies that the downloaded file size matches.
 * On success, returns a newly allocated string with the temporary file path.
 * On failure, returns NULL.
 */
static char *download_file(const char *url, const unsigned long expected_size, ProgressData *p_data)
{
    ...
    do {
        ...
        if (retry_count < max_retries) {
            fprintf(stderr, "Retrying in %d seconds... (retry %d of %d)\n",
 ...
 * 'expected_size' is the expected size of the decompressed file (if > 0 it is checked).
 *
 * Returns TRUE on success, FALSE on failure.
 * Note: unelzma is expected to remove the cabinet file on successful extraction.
 */
static gboolean extract_cabinet(const char *cabinet_path, const char *dest_path, unsigned long expected_size)
{
    char command[1024];
    /* Since unelzma is a custom app we build for use with the launcher it is expected
     * to be bundled with the launcher */
    snprintf(command, sizeof(command), "./unelzma \"%s\" \"%s\"", cabinet_path, dest_path);
    if (system(command) != 0)
        return FALSE;

    unsigned long actual_size = get_file_size(dest_path);
...
static gboolean download_version_ini(UpdateData *data) {
    ...
    gchar *version_ini_url = g_strdup_printf("%s/%s", data->public_patch_url, "version.ini");
    ...
    snprintf(dest_path, sizeof(dest_path), "%s/%s", current_dir, "version.ini");
    ...
    if (g_file_test(dest_path, G_FILE_TEST_EXISTS)) {
        if (!g_file_delete(dest_file, nullptr, &error)) {
            g_printerr("Unable to delete the old version.ini while fetching the new one.");
            g_error_free(error);
 ...
 * load_server_db:
 *
 * Downloads the latest database cabinet using the "DB file" value (db_cab_filename)
 * combined with data->public_patch_url, extracts it to a temporary file (with the .cab removed)
 * and opens the resulting SQLite database.
 *
 * Returns a pointer to an open sqlite3 database or NULL on error.
 *
 * NOTE: In a production environment the expected compressed file size should be taken from version.ini.
 */
static sqlite3 *load_server_db(UpdateData *data, gboolean skip_download)
{
    ...
    if (!skip_download) {
        ...
        gchar *db_url = g_strdup_printf("%s/%s", data->public_patch_url, db_url_path);
...
void updater_init() {
    ...
    GError *error = nullptr;
    generate_file_paths_gbytes = g_resources_lookup_data("/com/tera/launcher/generate-file-paths.sql", 0, &error);
    if (!generate_file_paths_gbytes) {
        g_error("Error loading SQL resource: %s", error->message);
    }

    generate_full_file_manifest_gbytes = g_resources_lookup_data("/com/tera/launcher/generate-full-file-manifest.sql", 0, &error);
    if (!generate_full_file_manifest_gbytes) {
        g_error("Error loading SQL resource: %s", error->message);
    }

    generate_update_manifest_gbytes = g_resources_lookup_data("/com/tera/launcher/generate-update-manifest.sql", 0, &error);
    if (!generate_update_manifest_gbytes) {
        g_error("Error loading SQL resource: %s", error->message);
    }

    generate_full_file_manifest_count_gbytes = g_resources_lookup_data("/com/tera/launcher/generate-full-file-manifest-count.sql", 0, &error);
    if (!generate_full_file_manifest_count_gbytes) {
        g_error("Error loading SQL resource: %s", error->message);
    }

    generate_file_paths_count_gbytes = g_resources_lookup_data("/com/tera/launcher/generate-file-paths-count.sql", 0, &error);
    ...
    sql_generate_update_manifest = g_bytes_get_data(generate_update_manifest_gbytes, &size);
    if (!sql_generate_update_manifest) {
        g_error("Could not get update manifest query data from resource.");
    }

    sql_generate_full_manifest = g_bytes_get_data(generate_full_file_manifest_gbytes, &size);
    if (!sql_generate_full_manifest) {
        g_error("Could not get full manifest query data from resource.");
    }

    sql_generate_full_manifest_count = g_bytes_get_data(generate_full_file_manifest_count_gbytes, &size);
    if (!sql_generate_full_manifest_count) {
        g_error("Could not get full manifest query count data from resource.");
    }

    sql_generate_file_paths_count = g_bytes_get_data(generate_file_paths_gbytes, &size);
...
static gboolean parse_version_ini() {
    ...
    if (!g_key_file_load_from_file(key_file, "version.ini", G_KEY_FILE_NONE, &error)) {
    ...
    current_version = g_key_file_get_integer(key_file, "Download", "Version", &error);
    if (error) {
        g_object_unref(key_file);
        return FALSE;
    }

    // If there are existing instances of these strings, we are clearing them before setting new values.
    if (db_url_path) {
        g_free(db_url_path);
        db_url_path = nullptr;
    }
    ...
    // Its possible the database will not end in cab, but let's handle the use case where it does for clarity.
    // If this does not occur, it does not materially impact the function of the launcher. Just the database
    // will be stored, uncompressed, in its unarchived form with possibly an archive looking name.
    ...
    // Honestly, if someone returns a suffix longer than the above for this, and we can't strip the suffix
    // off because of it, it's on them. This won't crash, but the db file gets to end in cab forever on the users
    // system :^).
 ...
 * get_files_to_update:
 *
 * Determines which game files need updating by comparing the local version with the latest
 * information in the server database. This function uses the generate-update-manifest SQL.
 *
 * Returns a GList of FileInfo structures (which must later be freed with free_file_info()).
 */
GList* get_files_to_update(UpdateData *data, ProgressCallback callback, gpointer user_data)
{
    ...
    if (!parse_version_ini()) {
        update_progress(callback, 0.0, "Missing or invalid version.ini: Beginning repair...", user_data);
        g_usleep(3000000);
        return get_files_to_repair(data, callback, user_data);
    }

    /* Fetch latest, continue with updates. We just check version.ini as a shortcut to know if things have
     * been messed with or otherwise failed to clear normally the last time the launcher ran.
     */
    if (!download_version_ini(data)) {
        update_progress(callback, 0.0, "Unable to fetch latest version.ini", user_data);
        return nullptr;
    }

    if (!parse_version_ini()) {
        update_progress(callback, 0.0, "Unable to parse latest version.ini", user_data);
        return nullptr;
    }
    ...
    if (!db) {
        update_progress(callback, 1.0, "Failed to download latest update database.", user_data);
        return update_list;
    }

    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db, sql_generate_update_manifest, -1, &stmt, nullptr) != SQLITE_OK) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        /* Construct the download URL using data->public_patch_url and naming convention:
           {public_patch_url}/{patch_path}/IDNUM-VERIDNUM.cab */
        info->url = g_strdup_printf("%s/%s/%d-%d.cab", data->public_patch_url, patch_path, id, new_ver);
 ...
 * get_files_to_repair:
 *
 * For repair operations (e.g. missing or damaged local files), this function queries the full
 * file manifest from the server database.
 *
...
{
    update_progress(callback, 0.0, "Checking for missing or damaged files...", user_data);
    ...
    if (!parse_version_ini()) {
        update_progress(callback, 0.0, "Unable to parse downloaded version.ini", user_data);
        return nullptr;
    }
    ...
    sqlite3 *db = load_server_db(data, FALSE);
    if (!db) {
        update_progress(callback, 1.0, "Failed to load server database.", user_data);
        return repair_list;
    }

    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db, sql_generate_full_manifest_count, -1, &stmt, nullptr) != SQLITE_OK) {
        g_printerr("SQL error: %s\n", sqlite3_errmsg(db));
        sqlite3_close(db);
        return repair_list;
    }
    ...
    if (sqlite3_prepare_v2(db, sql_generate_full_manifest, -1, &stmt, nullptr) != SQLITE_OK) {
        g_printerr("SQL error: %s\n", sqlite3_errmsg(db));
        sqlite3_close(db);
        return repair_list;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        snprintf(progress_msg, sizeof(progress_msg), "Scanning file %u of %i: %s",
                 processed, record_count, (const char*)file_name);
        g_free(file_name);
        update_progress(callback, (double)processed / (double)record_count, progress_msg, user_data);
        if (g_file_test((const gchar*)path_text, G_FILE_TEST_EXISTS)) {
            ...
            if (strcmp((const char*)hash_text, md5_result) == 0) {
                ...
                if (!busted_file) {
                    // TODO: Handle this better because ya know, if we can't replace this bad file then
                ...
                if (!g_file_delete(busted_file, nullptr, &error)) {
                    ...
                    g_printerr("Unable to delete '%s': %s", (const char*)path_text, error->message);
                    g_clear_error(&error);
                }
            }
                free(md5_result);
 ...
 * download_all_files:
 *
 * Given a list of FileInfo structures representing files to update, this function
 * downloads, extracts, validates, and writes each updated file.
 */
gboolean download_all_files(UpdateData *data, GList *files_to_update, ProgressCallback callback, ProgressCallback download_callback, gpointer user_data)
{
    ...
    // There will be cascading failures if this is not performed so we will simply return FALSE
    // for success if the directory tree check fails.
    update_progress(callback, 0.0, "Building game directory tree...", user_data);

    sqlite3 *db = load_server_db(data, TRUE);
    sqlite3_stmt *stmt = nullptr;
    if (sqlite3_prepare_v2(db, sql_generate_file_paths_count, -1, &stmt, nullptr) != SQLITE_OK) {
        g_printerr("SQL error: %s\n", sqlite3_errmsg(db));
        sqlite3_close(db);
        return FALSE;
    }
    ...
    if (sqlite3_prepare_v2(db, sql_generate_file_paths, -1, &stmt, nullptr) != SQLITE_OK) {
        g_printerr("SQL error: %s\n", sqlite3_errmsg(db));
        sqlite3_close(db);
        return FALSE;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        processed++;
        snprintf(progress_msg, sizeof(progress_msg), "Checking directory %u of %i: %s", processed,
            directories_count, (const char*)dir_path);
        update_progress(callback, (double)processed / directories_count, progress_msg, user_data);
        if (g_file_test((const gchar*)dir_path, G_FILE_TEST_EXISTS)) {
            ...
            if (remove((const char*)dir_path) != 0) {
                update_progress(callback, 1.0, "Failed to remove file where directory should be", user_data);
                sqlite3_finalize(stmt);
                sqlite3_close(db);
                return FALSE;
            }
    ...
    for (const GList *l = files_to_update; l != NULL; l = l->next) {
        ...
        processed++;
        snprintf(progress_msg, sizeof(progress_msg), "Downloading file %u of %u: %s",
                 processed, total_files, file_name);
        ...
        /* Download the cabinet file for the update.
           Validate that the downloaded file size matches the expected compressed size. */
        ProgressData p_data;
        ...
        /* In a production updater, the expected decompressed size would come from the manifest.
           Here, we pass 0 to indicate no size check (or update as needed). */
        snprintf(progress_msg, sizeof(progress_msg), "Extracting file %u of %u: %s",
                 processed, total_files, file_name);
        g_free(file_name);
        update_progress(callback, current_progress, progress_msg, user_data);

...
extern "C" {
    ...
    /**
     * @brief Fetches a server list via XML from a given URL, parses it into a Protobuf `ServerList`,
     *        and returns the serialized bytes. The caller must free() the returned buffer.
     *
     * @param[out] out_size         The number of bytes in the returned buffer.
     * @param[in]  server_list_url  The URL to fetch; must not be NULL.
     * @return Pointer to a newly allocated buffer of Protobuf data, or NULL on failure.
     */
